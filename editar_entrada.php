...
if (!esEditor()) {
    header("Location: index.php"); // Redirigir al índice si no tiene permisos
    exit;
}

$slug = $_GET['slug'] ?? ''; // Obtener el slug de la entrada desde la URL

if (!$slug) {
    header("Location: index.php"); // Redirigir si no se proporciona el slug
    exit;
}

$entrada = obtenerEntrada(sanitizar($slug)); // Obtener la entrada desde la base de datos

if (!$entrada) {
    header("HTTP/1.0 404 Not Found"); // Mostrar error 404 si no se encuentra la entrada
    echo "Entrada no encontrada";
    exit;
}

$titulo = $entrada['titulo']; // No escapar aún
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['cancelar'])) {
        header("Location: admin.php"); // Redirigir al cancelar
        exit;
    }

    $titulo = sanitizar($_POST['titulo'] ?? ''); // Sanitizar el título ingresado
    $contenido = $_POST['contenido'] ?? ''; // Obtener el contenido

    if ($titulo === '' || $contenido === '') {
        $error = "El título y el contenido no pueden estar vacíos."; // Validar campos vacíos
    } else {
        ...
        try {
            ...
            header("Location: admin.php"); // Redirigir al guardar exitosamente
            exit;
        } catch (Exception $e) {
            $error = "Error al guardar la entrada: " . $e->getMessage(); // Mostrar error al guardar
        }
    }
}
    ...
    <div class="edit-page-container">
        <div class="main-container">
            <header class="edit-page-header">
                <h1>Editar Entrada</h1> <!-- Título principal -->
            ...
            <form method="post">
                ...
                <div id="editor"><?php echo $contenido; ?></div> <!-- Contenedor de CKEditor -->
